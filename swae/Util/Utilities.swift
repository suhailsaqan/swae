...
class Utilities {
    ...
    /// Returns a truncated profile name for chat display (max 20 chars)
    func truncatedProfileName(publicKeyHex: String?, appState: AppState) -> String {
        let name = profileName(publicKeyHex: publicKeyHex, appState: appState)
        return name.smartTruncatedUsername()
    }
