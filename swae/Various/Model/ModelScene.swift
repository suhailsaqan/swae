...
extension Model {
    ...
    private func unregisterGlobalVideoEffects() {
        ...
        media.unregisterEffect(zapPlasmaEffect)
        ...
        zapPlasmaEffect = ZapPlasmaEffect()
    ...
    private func registerGlobalVideoEffects(scene: SettingsScene) -> [VideoEffect] {
        ...
        // Zap Plasma Effect - always registered when enabled, processes only during active zaps
        if stream.zapPlasmaEffectEnabled && stream.zapStreamCoreEnabled {
            effects.append(zapPlasmaEffect)
        }
    ...
    func getNostrChatEffect(id: UUID) -> NostrChatEffect? {
        return nostrChatEffects[id]
    }
    ...
    private func resetVideoEffects(widgets: [SettingsWidget]) {
        ...
        for nostrChatEffect in nostrChatEffects.values {
            media.unregisterEffect(nostrChatEffect)
        }
        nostrChatEffects.removeAll()
        for widget in widgets where widget.type == .nostrChat {
            let effect = NostrChatEffect(
                widget: widget.nostrChat,
                settingName: widget.name
            )
            effect.effects = widget.getEffects()
            nostrChatEffects[widget.id] = effect
        }
        if nostrChatEffects.isEmpty {
            stopNostrChatBridge()
        } else {
            startNostrChatBridge()
        }
        // Collab video: unregister and re-associate if call is active
        for collabEffect in collabVideoEffects.values {
            media.unregisterEffect(collabEffect)
        }
        collabVideoEffects.removeAll()
        if let compositor = guestCompositor {
            for widget in widgets where widget.type == .collabVideo {
                collabVideoEffects[widget.id] = compositor
            }
        }
    ...
    /// Adds a single widget's effect instance without tearing down all existing effects.
    /// Use this instead of resetSelectedScene when adding one new widget to avoid
    /// destroying expensive effects (WKWebView, VRM models) for other widgets.
    func addSingleWidgetEffect(widget: SettingsWidget) {
        switch widget.type {
        case .text:
            textEffects[widget.id] = TextEffect(
                format: widget.text.formatString,
        ...
        case .browser:
            let videoSize = media.getVideoSize()
            guard let url = URL(string: widget.browser.url) else { return }
            let browserEffect = BrowserEffect(
                url: url,
                styleSheet: widget.browser.styleSheet,
                widget: widget.browser,
            ...
            browserEffect.effects = widget.getEffects()
            browserEffects[widget.id] = browserEffect
            // Rebuild browsers array
            browsers = browserEffects.map { _, browser in
                Browser(browserEffect: browser)
            }
        case .image:
            guard let data = imageStorage.read(id: widget.id) else { return }
        ...
        case .snapshot:
            ...
            snapshotEffect.effects = widget.getEffects()
            snapshotEffects[widget.id] = snapshotEffect
        case .scene, .crop, .videoEffect:
            // Scene/Crop/VideoEffect don't have standalone effect instances
            break
        case .nostrChat:
            let effect = NostrChatEffect(
                widget: widget.nostrChat,
            ...
            startNostrChatBridge()
        case .collabVideo:
            // Collab video effect is created by activateCompositing() — just store the reference
            if let compositor = guestCompositor {
                collabVideoEffects[widget.id] = compositor
            }
        }
        // Refresh the pipeline so the new effect gets included
        sceneUpdated(attachCamera: isCaptureDeviceWidget(widget: widget))
    }

    /// Updates a widget's position directly on its effect instance without calling sceneUpdated().
    /// Used during drag-to-position to avoid browser snapshot timer thrashing and main thread saturation.
    /// Call sceneUpdated() once when the drag ends to fully reconcile.
    func updateWidgetPositionDirect(widgetId: UUID, x: Double, y: Double, width: Double, height: Double) {
        guard let scene = getSelectedScene(),
              let sceneWidget = scene.widgets.first(where: { $0.widgetId == widgetId }) else { return }

        // Update the model so it persists
        sceneWidget.x = x
        sceneWidget.y = y
        sceneWidget.width = width
        sceneWidget.height = height

        // Update the effect directly (bypasses full pipeline rebuild)
        guard let widget = findWidget(id: widgetId) else { return }
        let videoSize = media.getVideoSize()
        let cloned = sceneWidget.clone()

        switch widget.type {
        case .text:
            textEffects[widgetId]?.setPosition(x: x, y: y)
        case .browser:
            // Use updatePositionOnly to avoid restarting snapshot timer
            browserEffects[widgetId]?.updatePositionOnly(
                pixelX: toPixels(x, videoSize.width),
                pixelY: toPixels(y, videoSize.height)
            )
        case .image:
            imageEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .map:
            mapEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .videoSource:
            videoSourceEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .qrCode:
            qrCodeEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .scoreboard:
            padelScoreboardEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .snapshot:
            snapshotEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .vTuber:
            vTuberEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .pngTuber:
            pngTuberEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .nostrChat:
            nostrChatEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        case .collabVideo:
            collabVideoEffects[widgetId]?.setSceneWidget(sceneWidget: cloned)
        default:
            break
        }
    }
    ...
    private func sceneUpdatedOn(scene: SettingsScene, attachCamera: Bool) {
        ...
        // Ensure the nostr chat bridge is running if there are active nostrChat effects.
        // This handles cases where the bridge failed to start initially (e.g., appState
        // was nil, zapStreamCoreEnabled was false) and conditions have since changed.
    ...
    private func addSceneEffects(
        ...
        for sceneWidget in scene.widgets {
            ...
            case .nostrChat:
                if let nostrChatEffect = nostrChatEffects[widget.id] {
                    nostrChatEffect.setSceneWidget(sceneWidget: sceneWidget.clone())
                    effects.append(nostrChatEffect)
                }
            case .collabVideo:
                if let collabEffect = collabVideoEffects[widget.id] {
                    collabEffect.setSceneWidget(sceneWidget: sceneWidget.clone())
                    effects.append(collabEffect)
                }
            }
        }
    }
