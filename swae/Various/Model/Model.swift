...
final class Model: NSObject, ObservableObject, @unchecked Sendable {
    ...
    // Track whether audio lifecycle is managed by ContentView
    // When true, mic switching and audio route changes won't auto-attach audio
    var audioManagedByContentView = false
    ...
    /// Set to `true` when `stopAll()` destroys the Processor/VideoUnit during background.
    /// ContentView observes this to reset its `isCameraAttached` tracking and re-attach
    /// the camera when the user is on the camera screen.
    @Published var needsCameraReattach = false
    ...
    var guestCompositor: GuestVideoCompositor?
    var audioMixerService: AudioMixerService?
    var _collabPeerPubkey: String?
    var collabVideoEffects: [UUID: GuestVideoCompositor] = [:]
    var collabVideoWidgetId: UUID?
    @Published var collabSendWidgets: Bool = true
    @Published var collabSkipPipForWebRTC: Bool = true
    @Published var guestAudioVolume: Float = 1.0
    var isEndingCollabCall = false
    var collabDisconnectTimer: Timer?
    var collabInviteTimer: Timer?
    var collabIceTimer: Timer?
    var collabThermalObserver: NSObjectProtocol?
    var collabBackgroundObserver: NSObjectProtocol?
    var collabForegroundObserver: NSObjectProtocol?
    var collabBackgroundTask: UIBackgroundTaskIdentifier = .invalid
    ...
    // MARK: - Zap Stream Core Integration
    var zapStreamCoreApiClient: ZapStreamCoreApiClient?
    ...
    func refreshZapStreamCoreBalance(completion: (() -> Void)? = nil) {
        guard stream.zapStreamCoreEnabled, let appState else {
            completion?()
            return
        }
        ...
        let config = ZapStreamCoreConfig(
            ...
            streamTitle: stream.zapStreamCoreStreamTitle,
        ...
        balanceRefreshCancellable = client.getAccountInfo(appState: appState)
    ...
    /// Start polling balance every 5 seconds. Safe to call multiple times.
    ...
    var latestVideoStallWarningTime = ContinuousClock.now
    ...
    /// When false, `updateOrientationLock()` will not change `AppDelegate.orientationLock`,
    /// keeping the feed/wallet/profile screens in portrait.
    var isCameraScreenVisible: Bool = false

    /// Returns the orientation the camera wants without applying it.
    /// Used by ContentView to set `AppDelegate.orientationLock` on navigation.
    var cameraDesiredOrientationLock: UIInterfaceOrientationMask {
        if stream.portrait {
            return .portrait
    ...
    // MARK: - Zap Plasma Effect
    
    func updateZapPlasmaEffectSettings() {
        zapPlasmaEffect.setSettings(
            enabled: stream.zapPlasmaEffectEnabled,
            intensityMultiplier: stream.zapPlasmaEffectIntensityMultiplier,
            minimumAmount: stream.zapPlasmaEffectMinimumAmount,
            colorPreset: stream.zapPlasmaEffectColorPreset,
            soundEnabled: stream.zapPlasmaEffectSoundEnabled,
            soundVolume: stream.zapPlasmaEffectSoundVolume
        )
    }
    
    func triggerZapPlasmaEffect(amount: Int64) {
        // Allow triggering for testing even if zapStreamCoreEnabled is false
        // but still respect zapPlasmaEffectEnabled
        guard stream.zapPlasmaEffectEnabled else { return }
        zapPlasmaEffect.triggerZap(amount: amount)
    }
    ...
    @objc func handleDidEnterBackgroundNotification() {
        ...
        case .service(let keepChatRunning):
            disableScreenPreview()
    ...
    @objc func handleWillEnterForegroundNotification() {
        ...
        // the underlying URLSessionWebSocketTasks in a stale state.
        appState?.reconnectRelays()

        // Re-subscribe to own live event after relay reconnection if currently streaming
        if streaming, stream.zapStreamCoreEnabled,
           let userPubkey = appState?.keypair?.publicKey.hex {
        ...
        case .off:
            ...
            // Signal ContentView to re-attach camera. stopAll() destroyed the old
            // Processor/VideoUnit, and reloadStream() created a new one without
            // camera inputs. ContentView's isCameraAttached is stale — reset it.
            needsCameraReattach = true
        }
    }
    ...
    func updateOrientationLock() {
        ...
        // Only apply orientation lock when camera screen is visible.
        // When on feed/wallet/profile, orientation stays .portrait.
        if isCameraScreenVisible {
            AppDelegate.orientationLock = cameraDesiredOrientationLock
        }
        updateCameraPreviewRotation()
    }
    ...
    func detachCamera() {
        ...
        // Detach video only - audio is managed by ContentView
