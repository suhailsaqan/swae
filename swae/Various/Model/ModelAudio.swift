...
extension Model {
    ...
    func selectMicDefault(mic: SettingsMicsMic) {
        ...
        let isCollabActive = collabCallState.isConnected
        processorControlQueue.async {
            let session = AVAudioSession.sharedInstance()
            guard let inputPort = session.availableInputs?.first(where: { $0.uid == mic.inputUid })
            else {
                return
            }
            try? session.setPreferredInput(inputPort)
            guard let dataSourceId = mic.dataSourceId as? NSNumber,
                let dataSource = inputPort.dataSources?.first(where: {
                    $0.dataSourceID == dataSourceId
                })
            else {
                return
            }
            ...
            // Sync WebRTC's RTCAudioSession with the new input so the VPIO
            // AudioUnit picks up the mic change during an active collab call.
            if isCollabActive {
                ...
                print("🔔 [COLLAB] Synced RTCAudioSession to new mic: \(mic.name)")
            }
        }
        media.attachDefaultAudioDevice(builtinDelay: database.debug.builtinAudioAndVideoDelay)
    ...
    func keepSpeakerAlive(now: ContinuousClock.Instant) {
        ...
        guard
            let soundUrl = Bundle.main.url(
                forResource: "Alerts.bundle/Silence", withExtension: "mp3")
        else {
            return
        }
    ...
    private func getActiveAudioSessionMic() -> SettingsMicsMic? {
        ...
        if let dataSource = inputPort.preferredDataSource {
            ...
            if inputPort.portType == .builtInMic {
                ...
                builtInMicOrientation = getBuiltInMicOrientation(
    ...
    private func addAudioSessionBuiltinMics(
...
private func setBuiltInMicAudioMode(
    dataSource: AVAudioSessionDataSourceDescription, preferStereoMic: Bool
) throws {
    if preferStereoMic {
