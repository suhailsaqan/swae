...
final class Media: NSObject {
        ...
        case .rtmp:
            ...
            for destination in destinations where destination.enabled {
                let rtmpStream = RtmpStream(
                    name: destination.name, processor: processor, delegate: self)
        ...
        case .srt:
            if newSrt {
                srtStreamNew = SrtStreamNew(
                    processor: processor, timecodesEnabled: timecodesEnabled, delegate: self)
                srtStreamOld = nil
            } else {
                srtStreamNew = nil
    ...
    func srtStartStream(
        ...
        srtlaClient!.start(
            uri: url, timeout: reconnectTime + 1, dnsLookupStrategy: dnsLookupStrategy)
    }
    ...
    func updateAdaptiveBitrate(overlay: Bool, relaxed: Bool) -> ([String], [String])? {
        ...
        if isSrtStreamActive() {
            return updateAdaptiveBitrateSrt(
                overlay: overlay, relaxed: relaxed, is200MsTick: is200MsTick)
        } else if is200MsTick {
            if let rtmpStream {
    ...
    private func updateAdaptiveBitrateSrt(overlay: Bool, relaxed: Bool, is200MsTick: Bool) -> (
        [String], [String]
    )? {
        ...
        if adaptiveBitrate is AdaptiveBitrateSrtBela {
            return updateAdaptiveBitrateSrtBela(
                overlay: overlay, relaxed: relaxed, is200MsTick: is200MsTick)
        } else if is200MsTick {
    ...
    private func updateAdaptiveBitrateSrtBela(
        overlay: Bool,
        relaxed: Bool,
        is200MsTick: Bool
    ) -> ([String], [String])? {
    ...
    private func updateAdaptiveBitrateRtmp(overlay: Bool, rtmpStream: RtmpStream) -> (
        [String], [String]
    )? {
    ...
    private func updateAdaptiveBitrateRist(overlay: Bool, ristStream: RistStream) -> (
        [String], [String]
    )? {
    ...
    // MARK: - WebRTC Collab Forwarding

    /// Set the raw frame callback on VideoUnit for the WebRTC bridge.
    /// Pass nil to disable (solo streaming).
    func setOnRawFrameCaptured(_ callback: ((CVPixelBuffer, CMTime) -> Void)?) {
        processor?.video.onRawFrameCaptured = callback
    }

    /// Toggle whether WebRTC sends post-effects (with widgets) or pre-effects (raw camera).
    func setCollabSendWidgets(_ enabled: Bool) {
        processor?.video.collabSendWidgets = enabled
    }

    /// Toggle whether to skip the PiP overlay in the WebRTC frame (eliminates recursion).
    func setCollabSkipPipForWebRTC(_ enabled: Bool) {
        processor?.video.collabSkipPipForWebRTC = enabled
    }

    /// Get the AudioUnit for the AudioMixerService to register buffered audio.
    var audioUnit: AudioUnit? {
        processor?.audio
    }

    /// Set the guest audio volume for collab mixing (0.0–2.0).
    func setGuestAudioVolume(_ volume: Float) {
        processorPipelineQueue.async {
            self.processor?.audio.guestAudioVolume = volume
        }
    }

    /// Override the ignore-frames-after-attach window (used during collab reattach).
    func setIgnoreFramesAfterAttachSeconds(_ seconds: Double) {
        processorPipelineQueue.async {
            self.processor?.video.overrideIgnoreFramesAfterAttachSeconds = seconds
        }
    }
    ...
    func attachBufferedCamera(
        ...
        fillFrame: Bool,
        isLandscapeStreamAndPortraitUi: Bool = false,
        forceSceneTransition: Bool = false
    ) {
    ...
    func detachDefaultAudioDevice() {
        processor?.detachAudio()
    }
    ...
    private func makeVideoCompressionSettings(
        videoCodec: SettingsStreamCodec,
        videoBitrate: Int?,
        keyFrameInterval: Int?
    ) -> [String: Any] {
    ...
    func getLastVideoFrameTime() -> ContinuousClock.Instant? {
        return processor?.video.latestSampleBufferTime
    }
}
...
extension Media: ProcessorDelegate {
    func stream(audioLevel: Float, numberOfAudioChannels: Int, sampleRate: Double) {
        DispatchQueue.main.async {
            if becameMuted(old: self.currentAudioLevel, new: audioLevel)
                || becameUnmuted(
    ...
    func streamVideoEncoderResolution(resolution: CGSize) {
        // Resolution changed callback from adaptive bitrate
        // Can be used to update UI or log analytics
        logger.debug("video-encoder: Resolution changed to \(resolution.width)x\(resolution.height)")
    }
}
    ...
    func srtlaReady(port: UInt16) {
        processorControlQueue.async {
            if self.srtStreamOld != nil {
                do {
                    try self.srtStreamOld?.open(
                        self.makeLocalhostSrtUrl(
                            url: self.srtUrl,
                            port: port,
                    ...
                    DispatchQueue.main.async {
                        self.delegate?.mediaOnSrtDisconnected(
                            String(
                                localized: "SRT connect failed with \(error.localizedDescription)")
                        )
                    }
                }
            } else {
                self.srtStreamNew?.open(
                    streamId: self.makeStreamId(url: self.srtUrl),
