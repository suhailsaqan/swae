...
class RtmpStream {
    ...
    private func startConnectTimer() {
        ...
        let timer = DispatchWorkItem { [weak self] in
            ...
            logger.info("rtmp: \(self.name): Connect timeout")
            self.connection.socket.close(isDisconnected: true)
        }
        connectTimer = timer
        processorControlQueue.asyncAfter(deadline: .now() + 20, execute: timer)
    }
