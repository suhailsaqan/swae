...
public final class LivePlayerMoveGesture: UIPanGestureRecognizer {
    ...
    static var botLeftPosition: CGPoint {
        ...
        return CGPoint(
            x: margin + smallVideoWidth / 2,
            y: view.frame.height - view.safeAreaInsets.bottom - bottomBarMargin
        )
    }
    ...
    static var edgePositions: [CGPoint] {
        ...
        let botRight = CGPoint(
            x: view.frame.width - (margin + smallVideoWidth / 2),
            y: view.frame.height - view.safeAreaInsets.bottom - bottomBarMargin
        )
        let topLeft = CGPoint(
            x: margin + smallVideoWidth / 2,
            y: view.safeAreaInsets.top + bottomBarMargin
        )
        ...
        return [
            botLeftPosition, botRight, topLeft, topRight,
            hiddenBotLeft, hiddenBotRight, hiddenTopLeft, hiddenTopRight,
        ]
    }
    ...
    static var adjustedEdgePositions: [CGPoint] {
        ...
        let hiddenBotLeft = botLeftPosition.translated(
            by: .init(x: -hideAdjustment - extraSpacing, y: 0))
    ...
    @objc func gesture() {
        ...
        case .changed:
            view.frame.origin = .init(
                x: view.frame.origin.x + translation.x - oldTranslation.x,
                y: view.frame.origin.y + translation.y - oldTranslation.y
            )

            self.liveView?.showChevron =
                view.center.x < -50 || view.center.x > rootView.frame.width + 50
