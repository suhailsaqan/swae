...
// MARK: - Content-Only Wrapper for UIKit Hosting

/// Content-only wrapper for UIKit hosting. Same as LeftOverlayView but without the
/// Spacer() that would cause the hosting controller to expand to full height.
/// Positioning is handled by UIKit constraints on the hosting controller.
struct LeftStatusContentView: View {
    let model: Model
    @ObservedObject var database: Database
