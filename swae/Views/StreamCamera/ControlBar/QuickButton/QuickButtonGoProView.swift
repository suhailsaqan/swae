...
private struct GoProDeviceToggleView: View {
...
struct QuickButtonGoProView: View {
    ...
    var body: some View {
        GeometryReader { metrics in
            Form {
                if !model.database.goProDevices.devices.isEmpty {
                    Section {
                        ForEach(model.database.goProDevices.devices) { device in
                            GoProDeviceToggleView(device: device)
                        }
                    } header: {
                        Text("BLE Devices")
                    }
