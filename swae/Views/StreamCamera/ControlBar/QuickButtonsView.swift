...
struct QuickButtonsInnerView: View {
    ...
    private func zapPlasmaAction(state: ButtonState) {
        state.button.isOn.toggle()
        model.stream.zapPlasmaEffectEnabled = state.button.isOn
        model.setGlobalButtonState(type: .zapPlasma, isOn: state.button.isOn)
        model.updateZapPlasmaEffectSettings()
        model.sceneUpdated(updateRemoteScene: false)
    }
